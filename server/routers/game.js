...
module.exports = (io) => {
  io.of("/api/game").on("connection", async (socket) => {
    ...
    socket.on("answer", async (data) => {
      ...
      const roomId = await getRooms(null, null, roomcode);
      let users = [];
      if (isCorrect && time > 0) {
        users = await updateGradeUser(userId, 1 + time * 0.2, roomId[0].id);
      } else {
        users = await getUser(null, null, roomId[0].id);
      }

      io.of("/api/game")
        .in(roomcode)
        .emit("playerAnswered", { users, answerOwner: userId });
    });
